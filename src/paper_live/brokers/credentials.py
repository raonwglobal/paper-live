...
def parse_toss_credentials(material: str) -> TossCredentials:
    ...
    # legacy: client_id:client_secret:account_seq
    parts = text.split(":")
...
def parse_kb_credentials(material: str) -> KbCredentials:
    ...
    if text.startswith("{"):
        ...
        try:
            order_path = str(data.get("order_path", "/api/v1/ssqm1802"))
    ...
    order_path = parts[2] if len(parts) > 2 and parts[2] else "/api/v1/ssqm1802"
