...
class CorporateActionBook:
    ...
    VALID_TYPES = frozenset(
        {"SPLIT", "REVERSE_SPLIT", "CASH_DIVIDEND", "STOCK_DIVIDEND", "DELISTING", "LISTING", "RIGHTS"}
    )

    def __init__(self, actions: Sequence[CorporateAction] | None = None):
        self._actions: tuple[CorporateAction, ...] = ()
        self.replace(actions or ())
    ...
    def replace(self, actions: Iterable[CorporateAction]) -> None:
        ...
        for a in actions:
            ...
            if typ not in self.VALID_TYPES:
                raise ValueError(f"unsupported action_type: {typ}")
            if not a.symbol or not a.market:
                raise ValueError("market and symbol are required")
            if a.ratio is not None and a.ratio <= 0:
    ...
    def for_symbol(
        self, market: str, symbol: str, start: date | None = None, end: date | None = None
    ) -> tuple[CorporateAction, ...]:
        return tuple(
            a
            for a in self._actions
            if a.market == market.upper()
            and a.symbol == symbol.upper()
            and (start is None or a.action_date >= start)
            and (end is None or a.action_date <= end)
        )

    def adjustment_factor(self, market: str, symbol: str, as_of: date) -> Decimal:
        factor = Decimal("1")
        for a in self.for_symbol(market, symbol, end=as_of):
            if a.action_type == "SPLIT" and a.ratio:
                factor *= a.ratio
            elif a.action_type == "REVERSE_SPLIT" and a.ratio:
                factor /= a.ratio
        return factor
