...
class ExecutionAuditTrail:
    ...
    def record_preflight(
        ...
        *,
        ...
        rejected_by_symbol = {
            str(row.get("symbol", "")).strip(): reason for row, reason in result.rejected
        }
        ...
        for row in rows:
            ...
            record = ExecutionAuditRecord(
                ...
                portfolio_rank=rank,
                target_weight=None if target_weight is None else str(target_weight),
                risk_approved=approved is not None,
        ...
        if run_manifest_id and self.manifest_tracker is not None:
            ...
            self.manifest_tracker.bind_stage(
                ...
                RunArtifact(
                    stage="risk",
                    artifact_id="preflight-" + checksum[:20],
                    row_count=len(records),
                    status="completed" if not result.rejected else "rejected",
