...
class DailyFeatureEngine:
    ...
    def build(self, rows: Sequence[Mapping[str, Any]], *, decision_time: str) -> list[dict[str, Any]]:
        ...
        output=[]
        for (market,symbol), items in groups.items():
            ...
            for i,row in enumerate(items):
                ...
                momentum=None if i < self.config.momentum_window or closes[i-self.config.momentum_window] == 0 else price/closes[i-self.config.momentum_window]-1
                returns=[log(closes[j]/closes[j-1]) for j in range(max(1,i-self.config.volatility_window+1),i+1) if closes[j]>0 and closes[j-1]>0]
                volatility=(sum((x-(sum(returns)/len(returns)))**2 for x in returns)/len(returns))**0.5 if returns else None
                ...
                output.append({**row, "return_1d": None if prev is None or prev==0 else price/prev-1, "momentum": momentum, "volatility": volatility, "volume_ratio": None if not volume or not avg_vol else volume/avg_vol, "feature_decision_time": decision_time, "feature_version":"daily-features-v1"})
        return output
