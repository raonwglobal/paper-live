...
class DailyPriceNormalizer:
    ...
    @classmethod
    def normalize(cls, row: Mapping[str, Any], *, symbol: str, market: str, source: str, available_at: str) -> DailyPriceRecord:
...
class DailyDatasetBuilder:
    ...
    def build(self, rows: Iterable[DailyPriceRecord], *, as_of: str, dataset: str = "market/daily_prices") -> DatasetManifest:
        ...
        return self.storage.write_jsonl(dataset, [r.as_row() for r in ordered], as_of=as_of, schema_version="daily-price-v2")
...
class DailyPriceIngestionService:
    ...
    def collect(self, symbols: Sequence[str], *, start_date: date, end_date: date, market: str, source: str, available_at: str | None = None) -> DatasetManifest:
        ...
        for symbol in sorted(set(s.strip() for s in symbols if s.strip())):
            ...
            for item in payload:
                rows.append(DailyPriceNormalizer.normalize(item, symbol=symbol, market=market, source=source, available_at=available))
