...
class DockerPluginExecutor:
    ...
    def execute(self, plugin_root: str, entrypoint: str, request: dict[str, Any], policy: SandboxPolicy) -> dict[str, Any]:
        ...
        if policy.network_hosts:
            raise ContainerExecutionError("network allow-list requires a dedicated proxy; direct container networking is disabled")
        cmd = [
            "docker", "run", "--rm", "--network", "none",
            "--read-only", "--cap-drop", "ALL", "--security-opt", "no-new-privileges",
            "--pids-limit", "64", "--memory", f"{self.spec.memory_mb}m", "--cpus", str(self.spec.cpu),
            "--tmpfs", "/tmp:rw,noexec,nosuid,size=16m",
            "-v", f"{plugin_root}:/plugin:ro", self.spec.image,
            "python", f"/plugin/{entrypoint}",
        ]
        try:
            completed = subprocess.run(
                cmd, input=json.dumps(request), text=True, capture_output=True,
                timeout=self.spec.timeout_seconds, check=False,
            )
