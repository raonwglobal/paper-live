...
from .installer import InstalledPlugin, PluginInstaller
from .lifecycle import PluginLifecycle, PluginLifecycleError, PluginRecord
...
class PluginManager:
    ...
    def register_verified(self, manifest):
        ...
        try:
            ...
            self.lifecycle.add(
                PluginRecord(
                    manifest.id, manifest.version, manifest.source_commit or "untracked", manifest.sha256 or "untracked"
                )
            )
    ...
    def install_and_enable(
        self, source: RepositorySource, plugin_id: str, version: str, commit: str
    ) -> InstalledPlugin:
        ...
        self.lifecycle.add(
            PluginRecord(installed.plugin_id, installed.version, installed.commit, installed.artifact_sha256)
        )
        try:
