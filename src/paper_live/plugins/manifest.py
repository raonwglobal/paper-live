...
class PluginManifest:
    ...
    def validate(self) -> None:
        ...
        if not self.version.strip() or not self.name.strip():
            raise ValueError("version and name are required")
        entry = Path(self.entrypoint)
        if entry.is_absolute() or ".." in entry.parts:
            raise ValueError("entrypoint must stay inside plugin")
        if self.permissions.shell:
            raise ValueError("shell execution must be explicitly enabled outside the default policy")
...
def load_manifest(data: dict) -> PluginManifest:
    ...
    manifest = PluginManifest(
        ...
        skills=tuple(str(x) for x in data.get("skills", [])),
        permissions=PermissionPolicy(
            tuple(raw.get("network", {}).get("allow", [])),
            tuple(raw.get("secrets", {}).get("allow", [])),
            tuple(raw.get("filesystem", {}).get("read", [])),
            bool(raw.get("shell", False)),
            tuple(execution.get("allowed_modes", ["PAPER_SANDBOX", "VIRTUAL_BACKTEST"])),
        ),
        source_commit=(data.get("source") or {}).get("commit"),
        sha256=(data.get("integrity") or {}).get("sha256"),
    )
    manifest.validate()
    return manifest
