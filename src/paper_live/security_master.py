...
class SecurityMaster:
    ...
    def replace(self, securities: Iterable[Security]) -> None:
        ...
        for s in securities:
            ...
            if key in items: raise ValueError(f"duplicate security: {key}")
            items[key]=Security(symbol= symbol, name=s.name.strip(), market=s.market.upper(), country=s.country.upper(), currency=s.currency.upper(), asset_class=s.asset_class.upper(), active=s.active, exchange=s.exchange)
        self._items=items
