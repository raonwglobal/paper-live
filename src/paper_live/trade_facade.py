...
from .execution import ExecutionGateway, Fill, OrderSide, OrderType, PaperOrderRequest
...
class InternalTradeFacade:
    ...
    def intent_from_portfolio_row(
        row: dict[str, Any], *, account_value: Decimal, current_quantity: Decimal = Decimal("0"),
        broker: str = "toss", order_type: str = "MARKET", lot_size: Decimal = Decimal("1"),
    ) -> OrderIntent | None:
        ...
        if delta == 0:
            return None
        return OrderIntent(
            symbol=symbol, side="BUY" if delta > 0 else "SELL", quantity=abs(delta),
            order_type=order_type, price=(price if order_type.strip().upper() == "LIMIT" else None), broker=broker,
        )

    def to_paper_order(self, intent: OrderIntent) -> PaperOrderRequest:
        client_id = intent.client_order_id or f"paper-{uuid4().hex[:16]}"
        return PaperOrderRequest(intent.symbol, intent.normalized_side(), intent.quantity,
                                 intent.normalized_order_type(), intent.price, client_id)

    def to_broker_order(self, intent: OrderIntent) -> BrokerOrderRequest:
        return BrokerOrderRequest(symbol=intent.symbol, side=intent.normalized_side().value,
                                   quantity=intent.quantity, order_type=intent.normalized_order_type().value,
                                   price=intent.price)
    ...
    def _assess_risk(self, intent: OrderIntent, reference_price: Decimal,
                     portfolio_context: PortfolioRiskContext | None = None) -> RiskAssessment:
    ...
    def preview(self, intent: OrderIntent, reference_price: Decimal,
                *, portfolio_context: PortfolioRiskContext | None = None) -> OrderPreview:
        ...
        message = ("REAL_LIVE requires approval_id; call submit with order approval." if requires_approval
                   else "Risk passed; submit without approval_id on paper/virtual path.") if risk.approved else "Risk rejected; submit will fail until limits are adjusted."
        preview = OrderPreview(intent, self._estimate_notional(intent, px), self.currency, risk,
                               requires_approval, preview_id, mode.value, message)
    ...
    def submit(self, intent: OrderIntent, reference_price: Decimal, *, approval_id: str | None = None,
               preview_id: str | None = None, portfolio_context: PortfolioRiskContext | None = None) -> Fill | OrderResult:
        mode = self.controller.get_current_mode()
    ...
    def snapshot(self) -> dict[str, Any]:
        mode = self.controller.get_current_mode()
        return {"mode": mode.value, "has_broker_router": self.broker_router is not None,
