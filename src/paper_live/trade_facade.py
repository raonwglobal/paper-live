...
class OrderIntent:
    ...
    def normalized_side(self) -> OrderSide:
        ...
        if value not in {"BUY", "SELL"}: raise ValueError(f"invalid side: {self.side}")
        return OrderSide(value)
    def normalized_order_type(self) -> OrderType:
        value = self.order_type.strip().upper()
        if value not in {"MARKET", "LIMIT"}: raise ValueError(f"invalid order_type: {self.order_type}")
...
class InternalTradeFacade:
    ...
    @staticmethod
    def intent_from_portfolio_row(row: dict[str, Any], *, account_value: Decimal, current_quantity: Decimal = Decimal("0"), broker: str = "toss", order_type: str = "MARKET", lot_size: Decimal = Decimal("1")) -> OrderIntent | None:
        symbol = str(row.get("symbol", "")).strip()
        if not symbol or account_value <= 0 or current_quantity < 0 or lot_size <= 0: raise ValueError("symbol, account_value, current_quantity and lot_size must be valid")
        try:
            weight = Decimal(str(row.get("target_weight", "0")))
            price = Decimal(str(row.get("close", row.get("price", "0"))))
        except Exception as exc: raise ValueError("portfolio row has invalid target_weight or price") from exc
        if weight < 0 or weight > 1 or price <= 0: raise ValueError("target_weight must be in [0, 1] and price must be positive")
        target_quantity = ((account_value * weight) / price / lot_size).to_integral_value(rounding=ROUND_DOWN) * lot_size
        delta = target_quantity - current_quantity
        if delta == 0: return None
        return OrderIntent(symbol=symbol, side="BUY" if delta > 0 else "SELL", quantity=abs(delta), order_type=order_type, price=(price if order_type.strip().upper() == "LIMIT" else None), broker=broker)
    ...
    def _assess_risk(self, intent: OrderIntent, reference_price: Decimal, portfolio_context: PortfolioRiskContext | None = None) -> RiskAssessment:
        ...
        try:
            if portfolio_context is None: self.risk.approve(order, reference_price)
            else: self.risk.approve_portfolio(order, reference_price, portfolio_context)
            return RiskAssessment(True, "PASS")
        except (PermissionError, ValueError) as exc: return RiskAssessment(False, "REJECT", (str(exc),))
    def preview(self, intent: OrderIntent, reference_price: Decimal, *, portfolio_context: PortfolioRiskContext | None = None) -> OrderPreview:
        if intent.quantity <= 0: raise ValueError("quantity must be positive")
        if reference_price <= 0 and intent.price is None: raise ValueError("reference_price or limit price is required")
        px = reference_price if reference_price > 0 else (intent.price or Decimal("0"))
        mode = self.controller.get_current_mode(); risk = self._assess_risk(intent, px, portfolio_context)
    ...
    def _resolve_live_secret(self, intent: OrderIntent, *, capability: str = "order.create") -> str:
        if self.secret_broker is None: raise PermissionError("SecretBroker is required for REAL_LIVE submit")
        secret_id = self.broker_secret_ids.get(intent.broker)
        if not secret_id: raise PermissionError(f"no secret mapping for broker: {intent.broker}")
    ...
    def _adapter_for(self, broker: str):
        if self.broker_router is None: raise PermissionError("BrokerRouter is not configured")
        adapter = self.broker_router.adapters.get(broker)
        if adapter is None: raise PermissionError(f"broker adapter is not configured: {broker}")
        return adapter
    def _inject_credentials(self, intent: OrderIntent, *, capability: str = "order.create") -> object:
        material = self._resolve_live_secret(intent, capability=capability); adapter = self._adapter_for(intent.broker)
        apply = getattr(adapter, "apply_secret_material", None)
        if apply is not None: apply(material)
        return adapter
    def _submit_live(self, intent: OrderIntent, *, approval_id: str | None) -> OrderResult:
        if self.broker_router is None: raise PermissionError("BrokerRouter is not configured")
        if self.order_approval_gate is None or approval_id is None: raise PermissionError("explicit live approval_id is required")
        self.order_approval_gate.require(approval_id); adapter = self._inject_credentials(intent)
        try: return self.broker_router.submit(ExecutionEnvironmentMode.REAL_LIVE.value, intent.broker, self.to_broker_order(intent), approval_id)
        finally:
            clear = getattr(adapter, "clear_secret_material", None)
            if clear is not None: clear()
    def submit(self, intent: OrderIntent, reference_price: Decimal, *, approval_id: str | None = None, preview_id: str | None = None, portfolio_context: PortfolioRiskContext | None = None) -> Fill | OrderResult:
        mode = self.controller.get_current_mode()
        if preview_id is not None:
            stored = self._previews.get(preview_id)
            if stored is None: raise ValueError("unknown preview_id")
            if not stored.risk.approved: raise PermissionError("preview risk was not approved")
            intent = stored.intent
        risk = self._assess_risk(intent, reference_price, portfolio_context)
        if not risk.approved: raise PermissionError("; ".join(risk.violations) or "risk rejected")
        if mode is ExecutionEnvironmentMode.REAL_LIVE: return self._submit_live(intent, approval_id=approval_id)
        return self.gateway.execute(self.to_paper_order(intent), reference_price)
    def cancel(self, *, broker: str, order_id: str, approval_id: str | None = None, symbol: str | None = None) -> bool:
        mode = self.controller.get_current_mode()
        if mode is not ExecutionEnvironmentMode.REAL_LIVE: raise PermissionError("cancel via broker is only available in REAL_LIVE")
        if self.broker_router is None: raise PermissionError("BrokerRouter is not configured")
        if self.order_approval_gate is None or approval_id is None: raise PermissionError("explicit live approval_id is required")
        self.order_approval_gate.require(approval_id); intent = OrderIntent(symbol=(symbol or "-"), side="BUY", quantity=Decimal("1"), broker=broker); adapter = self._inject_credentials(intent, capability="order.cancel")
        try: return self.broker_router.cancel(mode.value, broker, order_id, approval_id, symbol=symbol)
        finally:
            clear = getattr(adapter, "clear_secret_material", None)
            if clear is not None: clear()
    def snapshot(self) -> dict[str, Any]:
        mode = self.controller.get_current_mode()
        return {"mode": mode.value, "has_broker_router": self.broker_router is not None, "has_secret_broker": self.secret_broker is not None, "has_order_approval_gate": self.order_approval_gate is not None, "open_previews": len(self._previews)}
