...
def test_gateway_repeated_filled_order_does_not_double_spend():
    gateway = ExecutionGateway(
        EnvironmentController(), VirtualMatchingEngine(PaperAccount(Decimal("10000")), slippage_bps=Decimal("0"))
    )
...
def test_gateway_partial_fill_only_executes_remaining_quantity():
    lifecycle_engine = VirtualMatchingEngine(
        PaperAccount(Decimal("10000")), slippage_bps=Decimal("0"), max_fill_ratio=Decimal("0.5")
    )
