...
def test_live_requires_explicit_approval():
    gate = LiveApprovalGate(); router = BrokerRouter({"fake": FakeBroker()}, gate)
    req = OrderRequest("005930", "BUY", Decimal("1"))
    with pytest.raises(LiveBrokerDenied): router.submit("REAL_LIVE", "fake", req)
...
def test_paper_never_reaches_broker():
    gate = LiveApprovalGate(); router = BrokerRouter({"fake": FakeBroker()}, gate)
    with pytest.raises(LiveBrokerDenied): router.submit("PAPER_SANDBOX", "fake", OrderRequest("005930", "BUY", Decimal("1")), None)
