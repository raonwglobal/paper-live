...
def runtime():
    ...
    lifecycle.add(PluginRecord("demo", "1.0", "abcdef1", "hash"))
    lifecycle.verify("demo")
    lifecycle.enable("demo")
    registry = SkillRegistry()
    registry.register(RegisteredSkill("signal", "demo", "1.0", "src/main.py"))
    executor = PluginSkillExecutor(registry)
    executor.bind("signal", lambda: {"signal": "BUY"})
...
def test_plugin_live_order_is_rejected():
    ...
    with pytest.raises(PermissionError):
        rt.submit_paper_order(request, order, Decimal("1000"))
