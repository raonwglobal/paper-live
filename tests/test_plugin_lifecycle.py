...
def test_verify_then_enable():
    life = PluginLifecycle(); life.add(record())
...
def test_unverified_plugin_cannot_enable():
    life = PluginLifecycle(); life.add(record())
    with pytest.raises(PluginLifecycleError): life.enable("demo")

def test_disable_and_remove():
    life = PluginLifecycle(); life.add(record()); life.verify("demo"); life.enable("demo")
    assert life.disable("demo").state == PluginState.DISABLED
    assert life.remove("demo").state == PluginState.REMOVED
    with pytest.raises(PluginLifecycleError): life.get("demo")
