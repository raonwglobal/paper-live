import pytest
from paper_live.plugins import PluginManifest, PluginRegistry, PermissionPolicy

def manifest(**kwargs):
    values = dict(id="demo-plugin", version="1.0.0", name="Demo", entrypoint="src/main.py", source_commit="abc", sha256="hash")
...
def test_unverified_plugin_cannot_enable():
    ...
    with pytest.raises(PermissionError): r.enable("demo-plugin")
...
def test_non_github_url_rejected():
    r = PluginRegistry()
    with pytest.raises(ValueError): r.add("http://evil.example/plugin", manifest())
