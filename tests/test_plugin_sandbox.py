...
def test_network_is_allow_listed():
    ...
    with pytest.raises(SandboxViolation): s.check_network("evil.example.com")
...
def test_filesystem_is_allow_listed():
    ...
    with pytest.raises(SandboxViolation): s.check_read("/etc/passwd")
    with pytest.raises(SandboxViolation): s.check_write("/tmp/x")

def test_shell_is_denied_by_default():
    with pytest.raises(SandboxViolation): PluginSandbox().check_shell()
