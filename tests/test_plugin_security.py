...
def test_manifest_url_requires_github_https():
    ...
    assert (
        loader.manifest_url(RepositorySource("https://github.com/acme/demo"))
        == "https://raw.githubusercontent.com/acme/demo/main/plugin.yaml"
    )
...
def test_manifest_shell_permission_is_rejected():
    ...
    with pytest.raises(PluginSecurityError):
        PluginSecurityValidator().validate_manifest(manifest)


def test_manifest_live_permission_is_rejected():
    manifest = PluginManifest(
        "demo-plugin", "1.0", "Demo", "src/main.py", permissions=PermissionPolicy(allowed_modes=("REAL_LIVE",))
    )
...
def test_plugin_policy_cannot_enable_live():
    with pytest.raises(PluginModeDenied):
        PluginExecutionPolicy({"REAL_LIVE"})
