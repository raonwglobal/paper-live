...
def test_paper_gateway_fail_closed_in_live_mode_even_after_approval():
    ...
    with pytest.raises(EnvironmentTransitionError):
        gateway.execute(OrderRequest("ABC", OrderSide.BUY, Decimal("1"), client_order_id="live-block"), Decimal("100"))
...
def test_risk_circuit_breaker_blocks_order():
    ...
    order = OrderRequest("ABC", OrderSide.BUY, Decimal("1"), client_order_id="risk-1")
    with pytest.raises(PermissionError):
        risk.approve(order, Decimal("100"))
    risk.circuit_breaker.trip()
    with pytest.raises(PermissionError):
        risk.approve(OrderRequest("ABC", OrderSide.BUY, Decimal("1"), client_order_id="risk-2"), Decimal("1"))
...
def test_paper_engine_rejects_insufficient_cash():
    ...
    with pytest.raises(ValueError):
        engine.execute(OrderRequest("ABC", OrderSide.BUY, Decimal("1"), client_order_id="cash-1"), Decimal("100"))
